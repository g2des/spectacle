...
\subsection{Dataset Creation}
The one of the key part of phase II is to generate a large enough dataset for analysis. As a part of this stage we identify news articles, that can potentially be called viral news stories. Based on these news articles we build a larger dataset comprising of all other relevant articles, metadata about these articles and the response these stories recieved on platforms like reddit and facebook. 
...
This list of urls is used to generate an intermediate dataset which consists of features like title, description and content of the articles. For extracting contents of the web pages, we use \emph{embed.ly}. It provides \emph{/1/extract/} API which can be used to retrieve article text, title, and other meta-data. This raw data is processed to remove html tags and html symbols.

\subsubsection{Scraping similar news articles}
To correctly identify the origin of a particular article, or to see the propagation path for an article, it is crucial to extract all the relevant articles of the web. In phase II, we used various APIs news aggregators like google news, NEWSAPI to identify more articles on the relevant news topic.To generate better results, the \emph{stop words} are removed.

\paragraph*{Stop Words}
Words that do not provide any discriminative power are referred to as stop words\cite{saif2014stopwords}. We used the classic method to remove the stop words. In this method, any word present in a precompiled list is removed from the text. We used Standford NLP's list of stop word\cite{stanfordnlp2019Dec}. 
